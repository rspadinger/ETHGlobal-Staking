...
export function StakeCard({ tokenSymbol: fallbackSymbol, tokenPrice, totalStaked }: StakeCardProps) {
    ...
    const handleStake = async () => {
        ...
        try {
            ...
            //@note execute approve and stakeTokens in same function
        ...
        <Card className="w-full max-w-md border border-slate-700 bg-slate-800 shadow-lg">
                ...
                <CardTitle className="text-xl flex items-center justify-between">
                    <div className="flex flex-col">
                        ...
                        <span className="text-sm text-muted-foreground">
                            Total staked: {totalStaked.toFixed(2)} {tokenSymbol} ($
                            {(totalStaked * tokenPrice).toFixed(2)})
                        </span>
                    </div>
                </CardTitle>
            </CardHeader>
            <CardContent className="pt-6 space-y-4">
                ...
                <div className="space-y-2">
                        ...
                        <label htmlFor="stake-amount" className="text-sm font-medium">
                            ...
                            <TooltipProvider>
                                <Tooltip>
                                    <TooltipTrigger>
                                        <TooltipInfo
                                            content={`Minimum stake amount: ${minimumStakeAmount} ${tokenSymbol}`}
                                        />
                                    </TooltipTrigger>
                                    <TooltipContent>
                                        <p>
                                            Minimum stake amount: {minimumStakeAmount} {tokenSymbol}
                                        </p>
                                    </TooltipContent>
                                </Tooltip>
                            </TooltipProvider>
                    ...
                    {amount && (
                        <p className="text-sm text-muted-foreground">
                            ≈ ${(Number.parseFloat(amount) * tokenPrice).toFixed(2)}
                        </p>
                    )}
                </div>
            </CardContent>
            <CardFooter className="flex flex-col gap-2">
                {isCheckingConnection ? (
                    <Button className="w-full bg-gray-600 hover:bg-gray-600 text-white cursor-wait" disabled={true}>
                        <span className="inline-flex items-center">
                            <LoadingSpinner />
                            <span className="ml-1">Checking wallet...</span>
                        </span>
                    </Button>
                ) : !isConnected ? (
                    <Button onClick={connect} className="w-full bg-primary hover:bg-primary/90 text-white">
                        Connect Wallet
                    </Button>
                ) : (
                    <>
                        ...
                        >
                            {isApproving ? (
                                ...
                                <span className="inline-flex items-center">
                                    <LoadingSpinner />
                                    <span className="ml-1">Staking...</span>
                                </span>
                            ) : (
                                "Stake"
                            )}
